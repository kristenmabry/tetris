...
#moves all the blocks above it down one row
				for block in self.all_blocks:
					...
					if upper_y/30 < row:
						block.move(0,30)
#should decrease each top by one but I don't know why it isn't working
				print(self.tops)
				for column in self.tops:	
					column += 1
				print(self.tops) #the tops haven't changed even though they should increase by one
...
# need to fix x
	def can_move(self,dx, dy):
		...
		can_move = True
		if (lowest_y/30)-1 + dy >= game.tops[int((right_x/30)-1)]:
			can_move = False
		elif dx != 0:
			if dx > 0 and right_x/30 + dx > game.column:
				can_move = False
			elif dx > 0 and lowest_y/30 >= game.tops[int((right_x/30)-1+dx)]:
				can_move = False
			elif dx < 0 and (right_x-30)/30 - dx <= 1:
				can_move = False
			elif dx < 0 and lowest_y/30 >= game.tops[int((right_x/30)-1-dx)]:
				can_move = False
		else:
